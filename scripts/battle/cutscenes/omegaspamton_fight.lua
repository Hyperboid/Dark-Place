...
function scenes.talk(cutscene, battler, enemy)
	...
	if spamton.health == spamton.max_health and enc.progress == 0 then
		cutscene:battlerText(spamton, {
			"THIS BODY HARDENS IN [PreSponse]\nTO [Magical] TRAUMA",
			"IN OTHER WORDS...[wait:5]\nYOU [Canned] HURT ME [Jack]"
		}, spamfiguration)
		enc.progress = 100
	elseif spamton.health < spamton.max_health and enc.progress == 100 then
		cutscene:battlerText(spamton, {
			"WHAT!? [Yuo're] [Killing Me]!?",
	...
	elseif enc.progress >= 200 and enc.progress < 299 then
		...
		if enc.progress == 205 then
			...
			cutscene:battlerText(spamton, {
				"HAEHAEHAEHAE!\nADORABLE, YOU THINK THAT'S\nA VALID [Options Trading]",
				"THIS BODY IS\nINDESTRUCTABLE, YOUR PLAN\nTO [Spare] ME TO [Heaven]\nWILL JUST [Nintendo NX]\nTO [Sleep Mode]."
			}, spamfiguration)
